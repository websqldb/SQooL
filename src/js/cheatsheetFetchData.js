  ...
  for (let i = 0; i < fileNameArray.length; i++) {
    const response = await fetch(
            ...
            sectionLi.innerHTML = `
              <button class="cheatsheet-clipboard-btn">
                <img
                  ...
                  alt="clipboard-button" width="15px"
                />
              </button>
      ...
      .then((data) => {
        ...
        // Cheatsheet Clipboard 복사기능 이벤트 추가
        const $cheatStatementBtn = document.querySelectorAll(
          ".cheatsheet-clipboard-btn"
        );
        ...
        $cheatStatementBtn.forEach((button) => {
          button.addEventListener("click", () => {
            if (
              button.nextElementSibling.classList.contains("cheatsheet-sql")
            ) {
